...
def fill_cell(table, j, nums_arr):
    ...
    for n in nums:
        ...
        if (checking.cell_check(table, row_index, col_index)):
            return 1
...
def new_game():
    ...
    assert(checking.is_full(table))
    assert(checking.is_solved(table))
...
def possibility_list(table):
    ...
    for j in range(81):
        row_index = j // 9
        col_index = j % 9
        if (checking.is_empty(table, row_index, col_index)):
